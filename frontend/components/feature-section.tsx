...
function SetupVisual() {
	...
	return (
		<div className="w-full flex flex-col items-stretch">
			<div className="relative mx-auto w-full max-w-[280px] rounded-xl border border-border/80 bg-muted/20 p-4 shadow-sm backdrop-blur-md overflow-hidden group/setup">
				{/* Decorative glowing gradient background */}
				<div className="absolute -top-12 -right-12 h-24 w-24 rounded-full bg-primary/10 blur-xl group-hover/setup:bg-primary/20 transition-all duration-500" />
				...
				<div className="flex items-center justify-between border-b border-border/40 pb-3 mb-3.5">
					...
					<span className="font-mono text-[9px] text-muted-foreground tracking-wide font-medium">academic_copilot.py</span>
					<div className="flex items-center gap-1">
						<span className="h-1 w-1 rounded-full bg-emerald-500 animate-pulse" />
						<span className="font-mono text-[8px] text-emerald-500 uppercase font-bold tracking-tight">Active</span>
					</div>
				</div>
				...
				<div className="space-y-2.5">
					{/* Step 1 */}
					<div className={cn(
						"flex items-center justify-between rounded-lg border p-2 bg-background/60 transition-all duration-300",
						activeStep === 0 ? "border-primary/50 shadow-xs scale-[1.02] bg-background" : "border-border/40"
					)}>
						<div className="flex items-center gap-2">
							<div className={cn(
								"flex h-5 w-5 items-center justify-center rounded-md border text-[10px] font-bold transition-all",
								activeStep >= 0 ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-500" : "bg-muted text-muted-foreground"
							)}>
								✓
							</div>
							<div className="text-[11px] text-left">
								<span className="block font-medium text-foreground">diagnose_gaps</span>
								<span className="block text-[8px] text-muted-foreground font-mono">Algebra drop detected</span>
							</div>
						</div>
						<span className="font-mono text-[8px] text-emerald-500/90 font-medium">0.4s</span>
					</div>

					{/* Step 2 */}
					<div className={cn(
						"flex items-center justify-between rounded-lg border p-2 bg-background/60 transition-all duration-300",
						activeStep === 1 ? "border-primary/50 shadow-xs scale-[1.02] bg-background" : "border-border/40"
					)}>
						<div className="flex items-center gap-2">
							<div className={cn(
								"flex h-5 w-5 items-center justify-center rounded-md border text-[10px] font-bold transition-all",
								activeStep >= 1 ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-500" : "bg-muted text-muted-foreground"
							)}>
								{activeStep >= 1 ? "✓" : "2"}
							</div>
							<div className="text-[11px] text-left">
								<span className="block font-medium text-foreground">compile_syllabus</span>
								<span className="block text-[8px] text-muted-foreground font-mono">Nightly study plan compiled</span>
							</div>
						</div>
						<span className="font-mono text-[8px] text-muted-foreground font-medium">
							{activeStep >= 1 ? "0.8s" : "pending"}
						</span>
					</div>
					...
					<div className={cn(
						"flex items-center justify-between rounded-lg border p-2 bg-background/60 transition-all duration-300",
						activeStep === 2 ? "border-primary/50 shadow-xs scale-[1.02] bg-background" : "border-border/40"
					)}>
						<div className="flex items-center gap-2">
							<div className={cn(
								"flex h-5 w-5 items-center justify-center rounded-md border text-[10px] font-mono transition-all",
								activeStep === 2 ? "bg-primary/10 border-primary/30 text-primary animate-pulse" : "bg-muted text-muted-foreground"
							)}>
								{activeStep === 2 ? "⚡" : "3"}
							</div>
							<div className="text-[11px] text-left">
								<span className="block font-medium text-foreground">fire_intervention</span>
								<span className="block text-[8px] text-muted-foreground font-mono">Proactive mentor ping dispatched</span>
							</div>
						</div>
						<span className="font-mono text-[8px] text-muted-foreground font-medium">
							{activeStep === 2 ? "running" : "queued"}
						</span>
					</div>
				</div>
				...
				<div className="mt-3.5 space-y-1">
					<div className="flex justify-between items-center text-[8px] font-mono text-muted-foreground">
						...
						<span>{activeStep === 0 ? "74%" : activeStep === 1 ? "86%" : "98%"}</span>
					</div>
					<div className="h-1 w-full bg-muted rounded-full overflow-hidden">
						<div 
							className="h-full bg-primary rounded-full transition-all duration-500 ease-out"
							style={{ width: activeStep === 0 ? "74%" : activeStep === 1 ? "86%" : "98%" }}
						/>
					</div>
				</div>
			</div>
			...
			<div className="relative mt-8 space-y-1.5 text-center">
				<FeatureTitle>Active Health Monitoring</FeatureTitle>
				<FeatureDescription>
					Sutra AI continuously computes scores, tracks mock tests, and fires nightly student interventions automatically.
				</FeatureDescription>
			</div>
		</div>
	);
...
function UserBasedSecurity() {
	return (
		<>
			...
			<div className="relative mt-8 space-y-1.5 text-center">
				<FeatureTitle>Weakness Detection Agent</FeatureTitle>
				<FeatureDescription>
					Sutra finds behavioral and cognitive patterns. It identifies deep root-cause deficiencies, such as basic division gaps causing algebra or ratio struggles.
				</FeatureDescription>
			</div>
		</>
	);
		...
		<>
			<div className="min-h-32">
				<div className="absolute top-8 left-8 flex items-center gap-2">
					...
					<div className="font-mono text-[10px] font-bold text-muted-foreground">Ready Score: 78%</div>
				</div>
				<ReportsChartsSvg className="translate-x-[5%] -rotate-2 scale-150" />
			</div>
			<div className="relative z-10 mt-8 space-y-1.5 text-center">
				<FeatureTitle>Exam Readiness Index</FeatureTitle>
				<FeatureDescription>
					Computes exact, expected scores, topic-specific confidence metrics, and delivers personalized practice paths on demand.
				</FeatureDescription>
			</div>
		</>
	);
			...
			<div className="relative z-10 space-y-6 py-8 ps-8 pe-2">
				...
				<div className="space-y-2">
					<FeatureTitle className="text-base">
						AI Board Paper Evaluator
					</FeatureTitle>
					<FeatureDescription>
						Students upload PDFs or direct handwritten response sheets. Sutra grades them according to precise state board marking rubrics.
					</FeatureDescription>
				</div>
			</div>
			{/* Dashboard Screen */}
		...
		<div className="grid max-h-120 sm:grid-cols-2">
			<div className="space-y-6 pt-8 pb-4 pl-8 sm:pb-8">
				...
				<div className="space-y-2">
					...
					<FeatureDescription>
						Mock paper difficulties adjust in real time according to accuracy. Sutra detects the exact current peak ability level automatically.
					</FeatureDescription>
				</div>
			</div>
